...
static void display_screen(_state_t* state) {
	char str[32];
	_float_t floatVoltage = toADCVoltage(state->sensor_data->maxVal);
...
static void display_screen_ppm(_state_t* state) {
	char str[32];
	_float_t floatVoltage = toADCVoltage(state->sensor_data->maxVal);
	_float_t float_ppm = voltage_to_ppm(floatVoltage);
...
static void display_screen_pct(_state_t* state) {
	char str[32];
	_float_t floatVoltage = toADCVoltage(state->sensor_data->maxVal);
	_float_t float_ppm = voltage_to_pct(floatVoltage);
